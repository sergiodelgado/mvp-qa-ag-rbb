...
function analyzeMigrationFile(filePath) {
  ...
  PROBLEMATIC_PATTERNS.forEach(({ pattern, issue, recommendation, severity }) => {
    ...
    if (matches) {
      // Verificar si ya está optimizado (dentro de SELECT)
      const isOptimized = content.includes(`(SELECT ${matches[0]})`);
...
async function run(ctx = {}) {
  ...
  if (requiresEnvVars) {
    ...
    if (missing.length > 0) {
      log.error(`Variables de entorno faltantes para validación de RLS: ${missing.join(', ')}`);
      return false;
    }
  }
  ...
  if (allIssues.length > 0) {
    ...
    allIssues.forEach((issue, index) => {
      ...
      console.log(`${index + 1}. ${severityColor}[${issue.severity.toUpperCase()}]${colors.reset} ${issue.file}`);
      console.log(`   Problema: ${issue.issue}`);
      console.log(`   Ocurrencias: ${issue.occurrences}`);
      console.log(`   Recomendación: ${colors.cyan}${issue.recommendation}${colors.reset}\n`);
    });
